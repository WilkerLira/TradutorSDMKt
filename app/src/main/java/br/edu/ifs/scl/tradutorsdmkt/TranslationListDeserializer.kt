...
import br.edu.ifs.scl.tradutorsdmkt.model.Translation
...
class TranslationListDeserializer : JsonDeserializer<List<Translation>> {
    override fun deserialize( // Função que desserializa o Json e retorna uma lista de objetos Translation
        json: JsonElement?,
        typeOfT: Type?,
        context: JsonDeserializationContext?
    ): List<Translation> {
        var results: JsonArray? = json?.asJsonObject?.getAsJsonArray("results")  // Recuperando o JsonArray “results” do Objeto completo Json
        val lexicalEntries: JsonArray = JsonArray() // Juntando os JsonArrays “lexicalEntries” de todos “results” num só JsonArray, O(N^2)
        results?.forEach {
            it?.let {
                lexicalEntries.addAll(it.asJsonObject?.getAsJsonArray("lexicalEntries"))
            }
        }
        val entries: JsonArray = JsonArray()  // Juntando os JsonArrays “entries” de todos “lexicalEntries” num só JsonArray, O(N^2)
        lexicalEntries?.forEach {
            it?.let {
                entries.addAll(it.asJsonObject?.getAsJsonArray("entries"))
            }
        }
        val senses: JsonArray = JsonArray()  // Juntando todos os JsonArrays “senses” de todos “entries” num só JsonArray, O(N^2)
        entries.forEach {
            it?.let {
